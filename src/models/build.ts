    ...
    static async getAll(projects: Project[]): Promise<Build[]> {
            ...
            return Object.entries(project.dockerCompose?.services ?? {}).flatMap(([serviceName, config]) => {
                if (config.build) {
                    ...
                    if (config.build && project.workingDirectory) {
                        const contextPath = typeof config.build === "string" ? config.build : config.build.context;
                        const workingDirectory = join(project.workingDirectory, contextPath);
                        const dockerfileName = (typeof config.build === "object" ?  config.build.dockerfile : undefined) ?? "Dockerfile";
                        return { serviceName, imageRef, project, config: { workingDirectory, path: join(workingDirectory, dockerfileName) } };
                    }
        ...
        return await Promise.all(builds.map(async (build) => {
            if (build.config?.path) {
                ...
                const config = { workingDirectory: build.config.workingDirectory, path: build.config.path, content };
